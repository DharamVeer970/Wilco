...
import { MicOff, Send, X, Volume2 } from "lucide-react";
      ...
      {isActive && isVisible && (
        ...
        >
          <div className="bg-slate-950/95 backdrop-blur-xl border border-purple-500/30 rounded-2xl shadow-[0_0_40px_rgba(168,85,247,0.2)] overflow-hidden">
            ...
            {/* Input area */}
            <div className="p-4">
              <div className="flex items-end gap-2 p-2.5 rounded-xl bg-slate-950/90 border border-purple-500/30 focus-within:border-purple-400 focus-within:ring-1 focus-within:ring-purple-500/40 transition shadow-inner">
                <textarea
                  ref={inputRef}
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  onKeyDown={handleKeyDown}
                  placeholder="Type message (emergency commands: 'help me', 'shutdown')..."
                  className="flex-1 min-h-[44px] max-h-32 p-2 bg-transparent text-slate-200 text-sm font-mono placeholder-slate-500 focus:outline-none resize-none overflow-y-auto scrollbar-thin [scrollbar-width:thin] [scrollbar-color:rgba(168,85,247,0.4)_transparent] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-purple-500/40 [&::-webkit-scrollbar-thumb]:hover:bg-purple-500/70 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent"
                ...
                <button
                  ...
                  title="Send message"
                  className="flex-shrink-0 p-2.5 rounded-lg bg-purple-500/20 hover:bg-purple-500/40 active:scale-95 border border-purple-500/30 text-purple-300 hover:text-purple-100 transition disabled:opacity-40 disabled:cursor-not-allowed mb-0.5"
                >
                  {isProcessing ? (
