...
const MEMORY_TABS: Array<MemoryCategory | "all"> = [
  "all",
  "identity",
  "preference",
  "goal",
...
export function MemoryDashboard({
  ...
  // ---------- Config ----------
  const categoryConfig: Record<MemoryCategory, { label: string; icon: LucideIcon; color: string; bg: string }> = {
  ...
  const handleManualAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logError("Memory inject failed:", err);
    } finally {
      setSubmitting(false);
    }
  };
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="absolute inset-0 m-4 max-w-350 mx-auto bg-black/90 backdrop-blur-3xl z-50 flex flex-col shadow-[0_0_100px_rgba(6,182,212,0.15)] overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center gap-2 overflow-x-auto px-6 py-2 bg-black/40 border-b border-cyan-500/10" role="tablist" aria-label="Memory partitions">
              {MEMORY_TABS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 text-[10px] font-mono uppercase tracking-widest border transition cursor-pointer shrink-0 ${
                      selected
                        ? "border-cyan-400 bg-cyan-500/20 text-cyan-200"
                        : "border-cyan-900/50 text-cyan-700 hover:border-cyan-500/40 hover:text-cyan-400"
                    }`}
                  >
                    {tab}
                  </button>
              ...
              {isAdding && (
                ...
                >
                  <form onSubmit={handleManualAdd} className="p-6 space-y-4 max-w-3xl mx-auto">
                    <span className="block text-[11px] font-mono tracking-widest text-cyan-600 uppercase mb-2">
                      Select Partition
                    </span>
                    ...
                    <label htmlFor="memory-payload" className="block text-[11px] font-mono tracking-widest text-cyan-600 uppercase mb-2">
                      Data Payload
                    </label>
                    <textarea
                      id="memory-payload"
                    ...
                    {filteredMemories.map((m) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className={`group relative p-px ${cfg.bg} transition-all duration-300 hover:-translate-y-1`}
                          style={{ clipPath: "polygon(20px 0, 100% 0, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0 100%, 0 20px)" }}
                        >
                          <div className="absolute inset-0 bg-linear-to-br from-cyan-500/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                          <div className="relative bg-black/80 backdrop-blur-xl p-5 border border-white/5 flex flex-col min-h-37.5" style={{ clipPath: "polygon(20px 0, 100% 0, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0 100%, 0 20px)" }}>
                            ...
                            <p className="text-sm text-cyan-50 font-mono wrap-break-word mb-3">{m.text}</p>
            ...
            <div className="p-3 border-t border-cyan-500/20 bg-black/60 flex items-center justify-between text-[9px] font-mono text-cyan-700 tracking-widest uppercase">
              <span className="flex items-center gap-2">
                ...
                <span>TERMINAL_LINK_ACTIVE</span>
              </span>
