...
type TranscriptFilter = "all" | "user" | "model";

const FILTER_CYCLE: Record<TranscriptFilter, TranscriptFilter> = {
  all: "user",
  user: "model",
  model: "all",
};

function nextFilter(current: TranscriptFilter): TranscriptFilter {
  return FILTER_CYCLE[current];
}

function getEntryTone(entry: TranscriptEntry): string {
  if (entry.isSelected) {
    return "bg-purple-500/20 border-purple-500/40 shadow-[0_0_20px_rgba(168,85,247,0.2)]";
  }
  if (entry.role === "user") {
    return "bg-linear-to-r from-cyan-900/20 to-transparent border-cyan-500/20 hover:border-cyan-500/40";
  }
  return "bg-linear-to-r from-purple-900/20 to-transparent border-purple-500/20 hover:border-purple-500/40";
}
...
export const TranscriptPanel: React.FC<TranscriptPanelProps> = ({
  ...
  const [activeFilter, setActiveFilter] = useState<TranscriptFilter>("all");
  const [isMinimized, setIsMinimized] = useState(false);
  const [isResizing, setIsResizing] = useState(false);
  const [panelWidth, setPanelWidth] = useState(520);
  const [panelHeight] = useState(600);
      ...
      >
        ...
        {!isMinimized && (
          <div
            ...
            aria-label="Transcript panel width"
            aria-valuenow={panelWidth}
            aria-valuemin={380}
            aria-valuemax={800}
            tabIndex={0}
            className="absolute left-0 top-0 bottom-0 w-2 bg-linear-to-b from-transparent via-purple-500/30 to-transparent cursor-col-resize z-50 hover:bg-purple-500/50 transition-colors"
            onMouseDown={(e) => {
              e.preventDefault();
              setIsResizing(true);
            }}
            onKeyDown={(e) => {
              if (e.key === "ArrowLeft") {
                e.preventDefault();
                setPanelWidth((w) => Math.max(380, w - 20));
              } else if (e.key === "ArrowRight") {
                e.preventDefault();
                setPanelWidth((w) => Math.min(800, w + 20));
              }
            }}
          />
        )}
        ...
        <div className="relative z-10 p-5 border-b border-white/5 bg-slate-950/70 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* Controls */}
            <button
              onClick={() => setActiveFilter(nextFilter(activeFilter))}
            ...
            {filteredEntries.length === 0 ? (
              ...
              filteredEntries.map((entry) => (
                <motion.div
                  ...
                  className={`relative group p-3.5 rounded-2xl border transition-all ${getEntryTone(entry)} ${entry.isError ? "border-rose-500/40" : ""}`}
                ...
                >
                  ...
                  {/* Entry content */}
                  <div
                    className="text-xs text-slate-200 font-mono leading-relaxed whitespace-pre-wrap wrap-break-word"
                  ...
                  {/* Selection indicator */}
                  {entry.isSelected && (
                    <div className="absolute top-0 left-0 bottom-0 w-1 bg-linear-to-b from-cyan-400 via-purple-400 to-cyan-400" />
                  )}
                </motion.div>
              ))
            )}
