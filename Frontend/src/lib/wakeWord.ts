 ...
 * entirely in the Wilco browser tab.
 *
 ...
 *
 * Public API:
 *   const det = new WilcoWakeWordDetector();
 *   det.start({ phrase, sensitivity, onTriggered, onState });
 *   det.setPhrase("hey wilco");
...
export class WilcoWakeWordDetector {
  ...
  setPhrase(phrase: string): void {
    this.phrase = (phrase || "hey wilco").toLowerCase().trim();
  }
      ...
      rec.onresult = (e: any) => {
        ...
        for (let i = e.resultIndex; i < e.results.length; i++) {
          ...
          for (const alt of res) {
            const raw = (alt as { transcript?: unknown })?.transcript;
            const transcript = (typeof raw === "string" ? raw : "").toLowerCase();
